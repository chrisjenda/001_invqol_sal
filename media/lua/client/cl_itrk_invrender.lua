...
local seenIcon = getTexture("media/ui/bookmark-seen.png"); --Salacious Added Seen Icon
-- endregion
                                    ...
                                    if first then
                                        ...
                                        elseif not cIt.isFood then
                                            ...
                                            elseif not ilTrait and checkType(item, constLiterature) and optionValues.itrk_EnableTrackLitItems then
                                                ...
                                                -- check Cached Table of already read books
                                                if readCheckCache[ft] then
                                                    -- If book is in the table then we have read it
                                                    mc.drawtexture(self, readIcon, var1, yy, 1, 1, 1, 1);
                                                elseif seenCheckCache[ft] then
                                                    if player:getAlreadyReadPages(ft) == 1 then
                                                        mc.drawtexture(self, seenIcon, var1, yy, 1, 1, 1, 1);
                                                    end
                                                elseif SkillBook[item:getSkillTrained()] then
                                                    if item:getNumberOfPages() == player:getAlreadyReadPages(ft) then
                                                        -- If we have read the whole book apply the read icon
                                                        mc.drawtexture(self, readIcon, var1, yy, 1, 1, 1, 1);
                                                        readCheckCache[ft] = true;
                                                    else
                                                        -- If player can read the book set the unread icon
                                                        if perkCache[ft] then
                                                            -- Checking if Player had read at least one page
                                                            if item:getNumberOfPages() > player:getAlreadyReadPages(ft) and
                                                            player:getAlreadyReadPages(ft) > 1 then
                                                                --getPlayer():Say(getText("testtesttest2"));
                                                                mc.drawtexture(self, poisonIcon, var1, yy, 1, 1, 1, 1)
                                                            else
                                                                -- If they have not read one page but can learn from the book mark seen but unread
                                                                if player:getAlreadyReadPages(ft) == 0 then
                                                            ...
                                                            if not (item:getLvlSkillTrained() > player:getPerkLevel(perk) + 1) and
                                                                ...
                                                                -- If Player can learn from the book
                                                                perkCache[ft] = true
                                                            else
                                                              -- Player cant Read the SkillBook
                                                              if player:getAlreadyReadPages(ft) == 1 then
                                                                  -- This book cannot be read, but has one page read so it must be seen already
                                                                  mc.drawtexture(self, seenIcon, var1, yy, 1, 1, 1, 1);
                                                                  seenCheckCache[ft] = true
                                                              end
                                                            end
                                                        end
                                                    end
                                                elseif rcpCheckCache[ft] then
                                                    ...
                                                    mc.drawtexture(self, aread and readIcon or rcpIcon,
                                                        var1, yy, 1, 1, 1, 1);
                                                    local aseen = tempCheckCache[ft] and tempCheckCache[ft] == 2
                                                    mc.drawtexture(self, aseen and seenIcon or rcpIcon,
                                                        var1, yy, 1, 1, 1, 1);
                                                    readCheckCache[ft] = aread
                                                    seenCheckCache[ft] = aseen
                                                    tempCheckCache[ft] = aread and aseen and 2 or 1 -- in render, you should check only once.
                                                end
                                            elseif cIt.isItem then
                                                if optionValues.itrk__EnableTrackMediaItems then
